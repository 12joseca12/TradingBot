...
// Calcula ATR simple a partir de series OHLC
...
// Decide si puede ejecutar lógica (rate limit simple por símbolo)
...
async function maybeOpenPaperTrade(user, settings, symbol, state) {
  ...
  // EV simple usando RR (rrTP) y SL unitario
  ...
  // Comprobar posiciones abiertas del usuario
...
// Inicia una sesión de trading para un usuario
export function startUserSession(user, settings, credsDecrypted) {
  ...
  const books = {}; // { symbol: { last, series:[], atr } }
  const lastRun = {}; // rate limit por símbolo
  ...
  ws.on("message", (data) => {
    try {
      ...
      // Creamos velas de 60s para ATR y ret
